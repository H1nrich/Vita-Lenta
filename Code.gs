...
function getAggregatedResults() {
  ...
  if (!sheet) {
    ...
    return res;
	// dentro getAggregatedResults, nella parte che scorre le righe:
rows.forEach(r => {
  // normalizza il valore: stringa, trim, case-sensitive match su AUTHORIZED
  const raw = r[2 + qi];
  const val = (raw === undefined || raw === null) ? '' : raw.toString().trim();
  if (!val) return;
  // trova corrispondenza autorizzata in modo case-insensitive
